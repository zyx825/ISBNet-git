...
class ISBNet(nn.Module):
    def __init__(
        ...
        if not self.semantic_only:
            ...
            # 这个网络块似乎被设计为处理输入特征，并输出一个共享的特征表示，这可以用于后续的实例分割、分类或者回归任务。
            self.inst_shared_mlp = GenericMLP(
                input_dim=self.channels * 4,   # 该input_dim 表示的是
            ...
            # 这个网络块可能负责基于共享特征表示进行实例的语义分类，即决定每个实例属于哪个类别。
            ...
            # 这个网络块设计用于预测实例的置信度，即每个检测到的实例是有效实例的概率。
            ...
            # 这个网络块用于预测每个实例的边界框或位置，通常在对象检测和实例分割任务中非常重要。
