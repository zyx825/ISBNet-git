...
class BatchNormDim1Swap(nn.BatchNorm1d):
    ...
    def forward(self, x):
        ...
        hw, n, c = x.shape
        x = x.permute(1, 2, 0)
        # 在下一步默认自定定义了num_features = 通道数c的BatchNorm1d
...
ACTIVATION_DICT = {
    "relu": nn.ReLU,  # f(x) = max(0, x)，线性整流单元
    "gelu": nn.GELU,  # xΦ(x)，高斯误差线性单元
}

WEIGHT_INIT_DICT = {
    "xavier_uniform": nn.init.xavier_uniform_, # 权重初始化方法，在训练深度神经网络时，帮助缓解梯度消失（vanishing gradients）或梯度爆炸（exploding gradients）的问题。
}
...
class GenericMLP(nn.Module):
    def __init__(
        ...
        super().__init__()
        activation = ACTIVATION_DICT[activation]   # 基本上都是使用relu激活函数
        norm = None   # 基本上使用到的地方都是使用bn1d归一化
    ...
    def do_weight_init(self, weight_init_name):
        ...
        for (_, param) in self.named_parameters():  # named_parameters()获取模型中所有参数的迭代器
