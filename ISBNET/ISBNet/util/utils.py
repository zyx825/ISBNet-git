...
class AverageMeter(object):
    ...
    def __init__(self, apply_dist_reduce=False):
        # apply_dist_reduce用于指示是否应用分布式数据并行
        self.apply_dist_reduce = apply_dist_reduce
        self.reset()

    def reset(self):
        # 这是一个实例方法，用于重置计数器和计算值的状态。它将val、avg、sum和count都重置为0。
        self.val = 0
        self.avg = 0
        self.sum = 0
        self.count = 0
    ...
    def dist_reduce(self, val):
        # 用于在分布式环境中对数值进行归约（reduce）。如果系统中有多个计算设备（如多个GPU），
        # 则可以使用该方法将值从各个设备归约到一个设备上。它使用了PyTorch的分布式工具dist来实现归约。
