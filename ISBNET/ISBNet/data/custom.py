...
class CustomDataset(Dataset):
    ...
    def __init__(self, data_root, prefix, suffix, voxel_cfg=None, training=True, repeat=1, logger=None):
        ...
        self.repeat = repeat
        self.logger = logger
        # self.mode = "train" if training else "test1"
        self.mode = "tea_for_train_pth" if training else "tea_for_test_pth"
    ...
    def collate_fn(self, batch):
        ...
        coords = torch.cat(coords, 0)  # long (N, 1 + 3), the batch item idx is put in coords[:, 0] ，将batch数据拼接到coords上，使得输入数据带batch值
